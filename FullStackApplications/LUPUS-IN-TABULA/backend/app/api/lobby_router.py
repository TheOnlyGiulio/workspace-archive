...
def _lobby_to_out(lobby) -> LobbyOut:
  return LobbyOut(
    ...
    spectators=[PlayerOut(id=p.id(), name=p.name()) for p in lobby.spectators()],
  )
...
def build_lobby_router(get_game_service):
  ...
  def start_game(
    ...
    try:
      ...
      raise HTTPException(status_code=403, detail="Only the lobby creator can start")
  ...
  def end_game(
    ...
    try:
      ...
      raise HTTPException(status_code=403, detail="Only the lobby creator can end")
    except ValueError as e:
      raise HTTPException(status_code=409, detail=str(e))
    except KeyError:
      raise HTTPException(status_code=404, detail="Lobby not found")
  ...
  def rematch(
    ...
    try:
      ...
      raise HTTPException(status_code=403, detail="Only the lobby creator can rematch")
    except ValueError as e:
      raise HTTPException(status_code=409, detail=str(e))
    except KeyError:
      raise HTTPException(status_code=404, detail="Lobby not found")
